...
\texttt{%
int x, sum = 100; \\
\textcolor{blue}{\bfseries \#pragma acc loop seq private(x) reduction(+:sum)} \\
for (int i = 0; i < 5; i++) \{ \\
\hspace*{2em} x = 100; \\
\hspace*{2em} sum += x; \\
\}
...
\texttt{%
int x; \\
\textcolor{blue}{\bfseries \#pragma acc loop seq reduction(+:x)} \\
for (int i = 0; i < 5; i++) \\
\hspace*{2em} x = 1;
}
\end{minipage} \\
% Arrows
\begin{minipage}{.5\textwidth}
\centerline{\rule[-0.5em]{2em}{0em} $\Downarrow$}
\end{minipage}
\hspace*{.07\textwidth}
\begin{minipage}{.36\textwidth}
\centerline{\rule[-0.5em]{2em}{0em} $\Downarrow$}
...
int x, sum = 100; \\
for (int i = 0; i < 5; i++) \{ \\
\hspace*{2em} \textcolor{blue}{\bfseries int x;} \\
\hspace*{2em} x = 100; \\
\hspace*{2em} sum += x; \\
\}
}
...
\texttt{%
int x; \\
\textcolor{blue}{\bfseries int sum = 0;\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ // (1)} \\
for (int i = 0; i < 5; i++) \{ \\
\hspace*{2em} \textcolor{blue}{\bfseries int x = 0;\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ // (2)} \\
\hspace*{2em} x = 1; \\
\hspace*{2em} \textcolor{blue}{\bfseries sum += x;\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ // (3)} \\
\} \\
\textcolor{blue}{\bfseries x = sum;\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ // (4)}
}
\end{minipage} \\
% Labels
\begin{minipage}{.5\textwidth}
\centerline{\rule[-0.5em]{2em}{0em} (a)}
\end{minipage}
\hspace*{.07\textwidth}
\begin{minipage}{.36\textwidth}
\centerline{\rule[-0.5em]{2em}{0em} (b)}
\end{minipage} \\
}
\end{center}
\caption{Elimination of \emph{private} and \emph{reduction} clauses on a sequential loop.}
\label{fig:privred}
\end{figure*}
...
the clause \texttt{reduction(+:x)}.  There are two cases.
...
}\emph{expression} (where \texttt{x} is not assigned in \emph{expression}),
...
\texttt{x}, and it is initialized to the initialization value for a sum
reduction (\texttt{0}).
(3)~At the end of the loop body, the local \texttt{x} is added to the result
(\texttt{sum}).
(4)~After the loop, the accumulated result is stored in the original
\texttt{x}.
